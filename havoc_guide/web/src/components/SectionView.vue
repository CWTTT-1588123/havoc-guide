...
const CAPS = { augments:'该英雄带这个符文胜率最高（按品质分档）', combos:'这几个符文一起带的胜率', synergy:'这两个符文一起带，胜率比该英雄平均▲高/▼低', builds:'该英雄这流派常见的核心三件套', counters:'敌方是这类阵容时，选这个符文克制（▲ 克制加成 / ▼ 反被压制）' }
...
function singleRow(a) { return `<div class="singlecard hasicon" data-name="${esc(a.name)}">${a.icon ? `<img class="sicon" src="${a.icon}" onerror="this.style.display='none'">` : '<span class="sicon ph"></span>'}<div class="sname">${esc(a.name)}${a.quality ? `<span class="q ${esc(a.quality)}">${esc(a.quality)}</span>` : ''}</div><div class="swr">${(a.wr * 100).toFixed(1)}%</div><div class="snum">${a.games}场</div></div>` }
function comboRow(c) { return `<div class="itemrow"><div class="l">${(c.augment_meta || []).map(m => augChip(m.name, m.quality, m.icon)).join('')} <span class="num">${c.games}场</span></div><span class="winrate">${(c.wr * 100).toFixed(1)}%</span></div>` }
...
function counterRow(s) { return `<div class="itemrow"><div class="l"><span class="tag">${esc(s.tag)}</span> ${augChip(s.augment_name, s.augment_quality, s.icon)} <span class="num">${s.games}场</span></div><span class="pct">${s.lift > 0 ? '▲' : '▼'} ${Math.abs(s.lift * 100).toFixed(1)}%</span></div>` }
...
const body = computed(() => {
  ...
  if (k === 'combos') return `<div class="list">${x.combos.map(comboRow).join('')}</div>`
