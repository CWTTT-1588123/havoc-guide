...
const CAPS = { augments:'该英雄带这个符文胜率最高（按品质分档）', combos:'这三个符文一起带的胜率（稳健估计，已按样本量收缩）', synergy:'这两个符文一起带，胜率比该英雄平均▲高/▼低', builds:'该英雄这流派常见的核心三件套', counters:'敌方是这类阵容时，选这个符文克制（▲ 克制加成 / ▼ 反被压制）' }
...
function comboRow(c) { const pct = c.est ?? c.wilson ?? c.wr; return `<div class="itemrow"><div class="l">${(c.augment_meta || []).map(m => augChip(m.name, m.quality, m.icon)).join('')} <span class="num num-desk">${c.games}场</span></div><div class="r"><span class="winrate">${(pct * 100).toFixed(1)}%</span> <span class="num num-mob">${c.games}场</span></div></div>` }
