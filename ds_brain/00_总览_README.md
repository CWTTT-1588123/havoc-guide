...
| 分析对局数(16.17) | **线上 62,195 场 / 621,950 记录**（2026-09-09 落袋）；本地数据池已再积累至 **~79,300 场**（未落袋 ~17,100，待下次 extract+stats） |
| 原始爬取文件(各版本) | data/ ~**3.1 万** 个（仍在增长） |
| 玩家池 | **25 万+**（BFS扩展；艾欧尼亚单区 23 万+，黑色玫瑰 5 万+） |
| 英雄 | 172 个 |
...
- **本地开发站**：需手动 `uvicorn`（后台任务易断）；**爬虫已恢复**：策略=**只爬艾欧尼亚双线**（a1_w1+a1_w2，0.3s），黑色玫瑰暂停；凭证 ~1.5h 过期，用**一键换凭证脚本** `recap_on.py`/`recap_off.py` 循环。
...
- **爬取重启 + 一键换凭证（2026-09-08/09）**：凭证过期 → 重抓包恢复爬取；策略改为**只爬艾欧尼亚双线**；新增 **`recap_on.py`/`recap_off.py` 一键换凭证脚本**（用户自己 PowerShell 跑：搭环境→点对局→清理+验证+自动合成凭证+自动拉起双线爬虫+监视器，全程中文提示）；`test_api.py` 改为自动取最新抓包；**9/9 落袋 62,195 场/62.2 万条已部署线上**（备份 `stats_backup_20260909.json`），本地继续积累 ~17,100 场未落袋。
- **待做**：百度站长平台提交 sitemap.xml 加快收录；本地 ~17,100 场跑 extract+stats 再落袋；凭证过期就用 recap_on/off 续爬（窗口随验证次数递减，隔夜恢复）。
