...
| 分析对局数(16.17) | **50,518 场** / 505,180 记录（头部英雄 ~7,400 场） |
...
| **评论/统计** | ✅ 评论区敏感词过滤（`sensitive_words.txt`）；管理后台「访问统计」（总/今日/近14天/页面分布，排除管理员浏览）+「评论管理」 |
...
- **待做**：百度站长平台提交 sitemap.xml 加快收录；黑色玫瑰饱和后轮换爬取更多大区；爬虫暂停中（用户指令，恢复需重抓凭证）。
