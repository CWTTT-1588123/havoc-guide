...
| 分析对局数(16.17) | **本地已落袋 84,397 场 / 843,970 条**（2026-09-09 晚；线上仍是 62,195 场，**待部署 +22,202 场**） |
| 原始爬取文件(各版本) | data/ ~**9.7 万** 个（仍在增长） |
| 玩家池 | **27 万+**（BFS扩展；艾欧尼亚双线各 27 万左右，黑色玫瑰暂停） |
| 英雄 | 172 个 |
...
- **待做**：把本地 84,397 场（stats.json）scp 部署上线（+22,202 场）；百度站长平台提交 sitemap.xml 加快收录；明晚凭证过期就 recap 换证 + `start_crawl.ps1` 续爬（窗口随验证次数递减，隔夜恢复）。
