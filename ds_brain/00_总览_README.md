...
- **组合胜率口径修复（2026-09-09 深夜）**：发现"最优符文组合"整列 100%（整套符文精确集合几乎不重复，2场小样本霸榜）→ 改为**任意 3 符文子集**统计（≥20 场，收缩+Wilson 排序），前端/AI 展示**稳健估计胜率**；stats.py/webapp.py/前端已改、`stats.json` 已重算（待部署），详见 08/03。
